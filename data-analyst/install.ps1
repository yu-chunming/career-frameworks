...
Write-Host "  数据分析师职业学习框架 - 安装脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan

function Test-OpenClaw {
    if (-not (Test-Path $OPENCLAW_HOME)) { Write-Host "[错误] 未检测到 OpenClaw 安装" -ForegroundColor Red; exit 1 }
...
function Install-Config {
    ...
    Copy-Item "$FRAMEWORK_DIR\career-config.json" "$CONFIG_DIR\data-analyst-config.json" -Force
    Write-Host "  [OK] 配置文件" -ForegroundColor Green
}

function Install-Skills {
    Write-Host "[2/3] 安装核心技能..." -ForegroundColor Yellow
    $skills = @("python", "writer", "summarize")
    foreach ($skill in $skills) { Write-Host "  [安装] $skill" -ForegroundColor Cyan; & openclaw skill install $skill 2>$null }
}
...
switch ($Action) {
    ...
    "uninstall" { Remove-Item "$CONFIG_DIR\data-analyst-config.json" -ErrorAction SilentlyContinue; Write-Host "[OK] 卸载完成" -ForegroundColor Green }
