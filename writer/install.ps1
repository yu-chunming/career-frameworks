...
Write-Host "  文字工作者职业学习框架 - 安装脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
function Install-Config {
    ...
    $configDst = "$CONFIG_DIR\writer-config.json"
    if ((Test-Path $configDst) -and -not $Force) {
        Write-Host "  [跳过] writer-config.json 已存在" -ForegroundColor Gray
    } else {
        Copy-Item $configSrc $configDst -Force
        Write-Host "  [OK] writer-config.json" -ForegroundColor Green
    }
}

function Install-Skills {
    Write-Host "[2/3] 安装核心技能..." -ForegroundColor Yellow
    $skills = @("writer", "summarize")
    foreach ($skill in $skills) {
        Write-Host "  [检查] $skill" -ForegroundColor Cyan
    }
    Write-Host "  [提示] 核心技能: writer, summarize, seo-writing" -ForegroundColor Yellow
}
...
function Show-NextSteps {
    ...
    Write-Host "  1. 编辑配置: code $CONFIG_DIR\writer-config.json" -ForegroundColor Gray
    Write-Host "  2. 查看技能: code $FRAMEWORK_DIR\SKILLS.md" -ForegroundColor Gray
    Write-Host "  3. 开始学习: openclaw skill learn writer" -ForegroundColor Gray
}

switch ($Action) {
    "install" { Test-OpenClaw; Install-Config; Install-Skills; Show-NextSteps }
    "uninstall" { Remove-Item "$CONFIG_DIR\writer-config.json" -ErrorAction SilentlyContinue; Write-Host "[OK] 卸载完成" -ForegroundColor Green }
    "status" { if (Test-Path "$CONFIG_DIR\writer-config.json") { Write-Host "[已安装]" -ForegroundColor Green } else { Write-Host "[未安装]" -ForegroundColor Red } }
